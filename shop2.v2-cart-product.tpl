											...
											{elseif $option.type == 'file2'}
												<li class="gr_type_file2">
													<div class="cart-params__item">
														<div class="cart-params__title">{$option.name}:</div>
														<div class="cart-params__value">
															<a href="{$FILES_DIR}{$op.filename}" class="{$shop2.my.gr_file|default:'gr-btn gr-btn-3 gr-btn-sm gr_btn_i_r'} file">
																<span>
																	{#SHOP2_DOWNLOAD#}
																	<i>
																		<svg class="gr-svg-icon">
		...
		<div class="cart-products__right">
			...
			{else}
				...
				<div class="cart-delete">
					{strip}
					<a class="cart-delete-btn" title="{#SHOP2_REMOVE#}" href="{$shop2.uri}?mode=cart&action=del&kind_id={$k}" data-id="
						{if $e.params}
							{ldelim}'{$k}':{ldelim}
