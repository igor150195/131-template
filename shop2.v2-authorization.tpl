<form method="post" class="tpl-form tpl-auth clear-self" action="{get_seo_url uri_prefix=$user_settings.link mode="login"}" method="post">
	...
	<div class="tpl-left">
		...
		<div class="tpl-btns">
			<button class="{$shop2.my.tpl_auth_enter_btn_class|default:'gr-btn gr-btn-1'}" type="submit">{#SHOP2_LOG_IN#}</button>
			{if $user_settings.registration_opened}
				<a class="{$shop2.my.tpl_auth_reg_btn_class|default:'gr-btn gr-btn-3'}" href="{get_seo_url uri_prefix=$user_settings.link mode="register"}">{#SHOP2_REGISTRATION#}</a>
			{/if}
		</div>
	...
	<div class="tpl-right">
		...
		<div class="tpl-field">
			<a class="{$shop2.my.tpl_auth_right_btn_class|default:'gr-btn gr-btn-3'}" href="{get_seo_url uri_prefix=$user_settings.link mode="forgot_password"}">
